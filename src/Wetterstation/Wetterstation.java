...
public class Wetterstation {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException, SQLException {
		//F¸r Testzwecke und eventuelle Erweiterungen nur auskommentiert und nicht gelˆscht
		//#########################################################################################
...
//		String defPlace = confData.getConfigFile();
		...
		/*if (place.equals("d")) {
			place = defPlace;
			System.err.println("Wetter f¸r den Defalut-Ort " + place + ":");
		...
		if (place != null && !place.equals("d")) {
			System.err.println("Wetter f¸r den Ort " + place + ":");
			readCurrentWeatherAPI(place);
			readForecastWeatherAPI(place);
			showCurrentWeather(place);
			showForecastWeather(place);
			...
			System.out.println("Eingabe ¸berpr¸fen");
		}*/
		//#########################################################################################
	...
	public static void showCurrentWeather(String place) {
		// Diese Methode belegt Werte von anderer API (WB) f¸r Durchschnittsberechnung
		showForecastWeatherToday(place);
		//#########################################################################################
		//Daten Openweathermap ausgabe ¸ber Gson
		...
		//#########################################################################################
		//Variablen werden mit Werten bef¸llt
		...
		//id wird als String angefordert, da sie f¸r den Pfad in ImageDownloader benˆtigt wird
	...
	public static void showForecastWeather(String place) {		
		// Daten Weatherbit.io Ausgabe ¸ber Gson
	...
	public static String requestPlace() {
		...
		System.out.print("Ort in ÷sterreich eingeben: ");
		place = sc.next();
	...
	public static void readCurrentWeatherAPI(String place) {
		...
		try {
			...
			dConn.connect();
		//#########################################################################################
			//Responsecode wird benˆtigt f¸r http requests --> 200 = Normalzustand (HTML und NWES Unterricht)
			int dResponsecode = dConn.getResponseCode();
			// System.out.println("Code: "+dResponsecode);
		//#########################################################################################
			if (dResponsecode != 200) { //200 ist code f¸r Normalzustand (NWES Unterricht)
		...
		//#########################################################################################
			...
			System.out
			.println("[readCurrentWeatherAPI] ungekl‰rter Fehler beim Einlesen der Current Weather API: " + e);
		}
		//#########################################################################################
	}
	public static void readForecastWeatherAPI(String place) {
		...
		//#########################################################################################
			if (fcResponsecode != 200) { //200 ist code f¸r Normalzustand (NWES Unterricht)
				/*
		...
		//#########################################################################################
			...
			System.out.println(
					"[readForecastWeatherAPI] ungekl‰rter Fehler beim Einlesen der Forecast Weather API: " + e);
		}
	}
	...
	public static String capitalizeFirstLetter(String s) {
		 ...
		 * 		legt fest, dass der erste buchstabe groﬂ ist
